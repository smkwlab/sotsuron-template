...
作成されたリポジトリで執筆を開始する。
下川研以外の利用者は、以下の手順2以降で自由に論文を執筆する。「3. Pull Request による添削システム」で説明している `0th-draftブランチ`は存在しないため無視すること。
下川研の利用者は、手順3以降のルールにしたがって執筆すること。
...
**重要**: PRは添削用のみ。PRはマージしてはならない。添削対応完了後は**自分でPRをクローズ**すること。
