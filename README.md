...
作成されたリポジトリで執筆を開始する。
下川研以外のユーザは、以下の 2. 以降、自由に論文を執筆する。3. で述べられている `0th-draftブランチ`は存在しないので無視すること。
下川研のユーザは、3. 以降のルールにしたがって、執筆すること。
...
**重要**: PRは添削用のみ。PRはマージしてはいけない。添削対応完了後は**自分でPRをクローズ**すること。
